				...
				foreach (string target in targetCurvesBinding.Keys)
				{
					...
					if (UseAnimationPointer && curve.propertyCurves != null && curve.propertyCurves.Count > 0)
					{
						...
						foreach (KeyValuePair<string, PropertyCurve> c in curves)
						{
							...
							{
								AddAnimationData(targetTr, prop.target, prop.propertyName, animation, AnimationInterpolationType.LINEAR, times, values);
					...
					if (targetTr)
					{
						...
						if (curve.translationCurves.Any(x => x != null))
						{
							...
							{
								AddAnimationData(targetTr, targetTr, trp2.propertyName, animation, AnimationInterpolationType.LINEAR, times, values2);
								sampledAnimationData = true;
							}
						}
						...
						if (curve.rotationCurves.Any(x => x != null))
						{
							...
							{
								AddAnimationData(targetTr, targetTr, trp3.propertyName, animation, AnimationInterpolationType.LINEAR,times, values3);
								sampledAnimationData = true;
							}
						...
						if (curve.scaleCurves.Any(x => x != null))
						{
							...
							{
								AddAnimationData(targetTr, targetTr, trp4.propertyName, animation, AnimationInterpolationType.LINEAR,times, values4);
								sampledAnimationData = true;
							}
						}
						...
						if (curve.weightCurves.Any(x => x.Value != null))
						{
							...
							if (BakePropertyAnimation(trp5, clip.length, AnimationBakingFramerate, speedMultiplier, out times, out var values5))
							{
								...
								AddAnimationData(targetTr, targetComponent, trp5.propertyName, animation, AnimationInterpolationType.LINEAR,times, values5);
								sampledAnimationData = true;
							}
						}
					}
